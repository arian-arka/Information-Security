...
def _encrypt(_data,_key):
    ...
    def strToHex(text):
        hexStr = ''
        for ch in text:
            hexStr += str(hex(ord(ch)))[2:]
        return hexStr
    def bitsToBytes(bits):
        l = []
        for i in range(len(bits) // 8):
            l.append(int(str(bits[i * 8:i * 8 + 8]), 2))
        return l

    key = hex2bin(strToHex(_key))

    pt = hex2bin(strToHex(_data))
    keyLength =len(key)
    if keyLength< 64:
        for i in range(64-keyLength):
            key += '0'
    else:
        key = key[:64]
    ...
    return base64.b64encode(bytes(bitsToBytes(encrypt(pt, rkb, rk)))).decode('utf-8')
...
def eachEncryption(start,end,thread):
    print('thread', thread)
    data = 'Break me'
    base64Cipher = 'OsQFp1sqAEo='
    for i in range(start,end):
        #os.system('cls')
        password = str(i)
        encrypted = _encrypt(data,password)
        #print(base64Cipher,encrypted)
        if  encrypted == base64Cipher:
            print(password)
